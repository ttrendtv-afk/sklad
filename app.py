...
def import_phones():
    ...
    if request.method == 'POST':
        ...
        # Use manual price if provided, otherwise calculate from cost
        ...
        added, skipped = 0, 0
        # Extract all digit sequences of length 10-16 (IMEI format)
        import re
...
def reports():
    ...
    net_usd = round(net_gel / gel_rate, 2) if gel_rate else 0
    ...
    return render_template('reports.html', phones=phones, tab=tab, models=models, stores=stores,
                           ...
                           gel_rate=gel_rate, usd_rate=usd_rate, states=states,
...
# ─── API REPORT STATS (dynamic, respects filters) ─────────────────────────────
...
def api_report_stats():
    ...
    def filter_q(status, date_field):
        q = Phone.query.filter_by(status=status)
        if sel_store: q = q.filter(Phone.store_id == sel_store)
        if sel_model: q = q.filter(Phone.model_id == sel_model)
        if sel_state: q = q.filter(Phone.state == sel_state)
    ...
    sold = filter_q('продан', Phone.date_sold)
    returns = filter_q('возврат', Phone.date_returned)
