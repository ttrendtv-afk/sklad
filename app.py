...
def add_phone():
    ...
    if request.method == 'POST':
        ...
        p = Phone(
            ...
            imei=imei, state=request.form.get('state', 'НОВАЯ'),
...
def reports():
    ...
    allowed = ['stock', 'sales', 'returns']
    if session.get('role') in ('admin', 'manager'):
        allowed = ['stock', 'sales', 'returns', 'finance']
    ...
    return render_template('reports.html', phones=phones, tab=tab, models=models, stores=stores,
                           ...
                           gel_rate=gel_rate, can_finance=can_finance, allowed_tabs=allowed)
...
def reports_export():
    ...
    if can_finance:
        writer.writerow(['Модель','IMEI','Магазин','Состояние','Себест.$','Наценка%','Курс','Цена ₾','Цена продажи ₾','Прибыль ₾','Дата приёма','Дата продажи','Дата возврата','Покупатель','Заметки'])
        for p in phones:
        ...
        writer.writerow(['Модель','IMEI','Магазин','Состояние','Цена ₾','Дата приёма','Дата продажи','Заметки'])
        for p in phones:
