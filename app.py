...
import os, json, csv, io, re, requests as http_requests
...
def changelog():
    ...
    logs = ChangeLog.query.order_by(ChangeLog.id.desc()).paginate(page=page, per_page=50, error_out=False)
...
def references():
    ...
    return render_template('references.html', models=models, stores=stores,
...
@app.route('/references/telegram', methods=['POST'])
@role_required('admin')
def save_telegram():
    set_setting('tg_token', request.form.get('tg_token', '').strip())
    set_setting('tg_chat_id', request.form.get('tg_chat_id', '').strip())
    # Test message
    send_telegram('✅ WMS Склад — Telegram уведомления подключены!')
    flash('Telegram настройки сохранены. Проверьте сообщение в боте.', 'success')
    return redirect(url_for('references'))
